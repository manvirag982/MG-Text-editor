...
main_application.title("MV Text Editor")
...
voice_icon=tk.PhotoImage(file=r"icons2\voice.png")

voice_btn=ttk.Button(tool_bar,image=voice_icon)
voice_btn.grid(row=0,column=9,padx=3)
...
def voice_changer():
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            # text_writer.insert(INSERT,"fdsf")
            text_writer.insert(tk.END,text)
        except:
            return 
            messagebox.showwarning('warning','Please Try Again , Not audible')

voice_btn.configure(command=voice_changer)
